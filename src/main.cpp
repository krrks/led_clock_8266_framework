// main.cpp — LED Matrix Clock v1.008
// Wemos D1 Mini (ESP8266) + 32×8 WS2812B + 4 buttons + onboard LED
//
// ── KEY CHANGE v1.008 ────────────────────────────────────────────────────
// WS2812B output switched from FastLED (bit-bang, WiFi-unsafe) to
// NeoPixelBus DMA/I2S (WiFi-safe). Data line moved to GPIO3 (RX/D9).
// Serial started with SERIAL_TX_ONLY so GPIO3 is free for DMA.
// ⚠ Rewire: D2 → RX(D9) for matrix data.
// ─────────────────────────────────────────────────────────────────────────
//
// Button layout:
//   BTN1 MODE    GPIO5  D1
...
// NeoPixelBus DMA strip — GPIO3 (RX/D9) is hardwired by the DMA method.
NeoPixelBus<NeoGrbFeature, NeoEsp8266Dma800KbpsMethod> neoStrip(NUM_LEDS);

// Logical pixel buffer (packed 0x00RRGGBB)
...
// ── Brightness ────────────────────────────────────────────────────────────
// applyBrightness() is a no-op here; brightness is applied per-pixel in
// flushDisplay() by reading configManager.data.brightness at render time.
void applyBrightness() {
    pendingRedraw = true;  // next flushDisplay() picks up the new value
}

// ── Scroll frame interval ─────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────
void setup() {
    // SERIAL_TX_ONLY frees GPIO3 (RX) so NeoPixelBus DMA can use I2S.
    ...
    // ── NeoPixelBus DMA init ────────────────────────────────────────────
    neoStrip.Begin();
    neoStrip.ClearTo(RgbColor(0, 0, 0));
    neoStrip.Show();
    Serial.printf("[LED] NeoPixelBus DMA on GPIO3, %d pixels\n", NUM_LEDS);

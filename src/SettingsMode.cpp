...
// ─── Active-item list ─────────────────────────────────────────────────────
// Skips SI_HOUR..SI_WD when NTP is synced (no point editing manual time).
void buildActiveSettings() {
    settingsCount = 0;
    for (int i = 0; i < SI_COUNT; i++) {
        if (i <= SI_WD && ntpSynced) continue;
...
// ─── Adjust one setting item by delta (+1 / -1) ───────────────────────────
void adjustSetting(int idx, int delta) {
    ...
    switch (idx) {
        ...
        case SI_BDIM: d.brightDim = (uint8_t)constrain((int)d.brightDim + delta * 5, 1, 200); applyBrightness(); break;
        case SI_BMED: d.brightMed = (uint8_t)constrain((int)d.brightMed + delta * 5, 1, 255); applyBrightness(); break;
        case SI_BBRT: d.brightBrt = (uint8_t)constrain((int)d.brightBrt + delta * 5, 1, 255); applyBrightness(); break;
        case SI_WX:
...
// ─── Draw current item onto the display matrix ────────────────────────────
...
void saveAndExitSettings() {
    ...
    // Re-anchor manual time base (only meaningful when NTP unavailable)
    if (!ntpSynced) {
